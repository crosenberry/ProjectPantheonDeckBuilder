using NUnit.Framework;
using Pantheon.Core.Combat;

namespace Pantheon.Core.Tests.Combat.Effects
{
    public class ConsumeStormPlusVolleyBonusDamageToAllEnemiesEffectTests
    {
        [Test]
        public void Apply_DealsCombinedScaledDamageToAllEnemies()
        {
            var player = new Player(startingEnergy: 3);
            player.GainStorm(2);
            player.GainVolley(3);
            var enemyA = new Enemy(maxHP: 42);
            var enemyB = new Enemy(maxHP: 42);
            var context = new CardEffectContext(player, enemyA, new[] { enemyA, enemyB });
            var effect = new ConsumeStormPlusVolleyBonusDamageToAllEnemiesEffect(damagePerStorm: 3, bonusPerVolley: 2);

            effect.Apply(context);

            Assert.That(enemyA.CurrentHP, Is.EqualTo(30));
            Assert.That(enemyB.CurrentHP, Is.EqualTo(30));
        }

        [Test]
        public void Apply_ConsumesStormButLeavesVolleyUntouched()
        {
            var player = new Player(startingEnergy: 3);
            player.GainStorm(2);
            player.GainVolley(3);
            var enemy = new Enemy(maxHP: 42);
            var context = new CardEffectContext(player, enemy);
            var effect = new ConsumeStormPlusVolleyBonusDamageToAllEnemiesEffect(damagePerStorm: 3, bonusPerVolley: 2);

            effect.Apply(context);

            Assert.That(player.Storm, Is.EqualTo(0));
            Assert.That(player.Volley, Is.EqualTo(3));
        }
    }
}
