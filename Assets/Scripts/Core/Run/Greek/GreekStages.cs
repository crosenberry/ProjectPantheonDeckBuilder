namespace Pantheon.Core.Run.Greek
{
    public static class GreekStages
    {
        ...
        // Combat -> Rest -> Combat -> Boss path. Real node counts/branching are
        // content tuning, deferred per the M4 design session.
        public static Stage SampleStage()
        {
            var nodes = new[]
            {
                new MapNode(NodeType.Combat, new[] { 1 }),
                new MapNode(NodeType.Rest, new[] { 2 }),
                new MapNode(NodeType.Combat, new[] { 3 }),
