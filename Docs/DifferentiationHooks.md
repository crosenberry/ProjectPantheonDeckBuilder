...
Prototype Hook 1 at the same fidelity as the existing Blessing work — rule shapes and a handful of concrete example cards/relics, not tuned numbers — using the Artemis/Thor pair. That gives a first concrete answer to "does combining two mythologies actually feel different from picking one," which is the real test of whether this closes the originality gap.
