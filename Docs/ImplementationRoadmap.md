...
### M7 — Sun Wukong design session (complete)

Fourth and final Blessing, same shape as M5/M6's sessions: [SunWukong-FullCardDraft.md](Characters/SunWukong-FullCardDraft.md) (75 cards) and [SunWukong-PassiveTree.md](Characters/SunWukong-PassiveTree.md) (17 nodes, same template) both predate the TDD process. This session curated a playable slice, drafted the missing Chinese enemy set, and resolved one genuine new-mechanic gap — bigger than Anubis's, since it touches nearly a whole archetype path rather than one starter card.

**Already locked — confirmed, not re-decided:** Form's rule shape (a 3-state stance — Mortal/Beast/Immortal, purely categorical, no magnitude; Beast doubles Attacks that normally hit once and adds 1 Energy to Skills; Immortal adds 1 to Strength/Block gained from cards and reduces Attack damage by 3, minimum 1 — [GDD §3.4](GameDesignDocument.md)), Wukong's three archetypes (Beast Rush/Immortal Ascension/72 Changes, plus Flex), the ruling that multi-hit cards are unaffected by Beast Form's doubling (resolved in the full draft itself), and the "safety net" Rare pattern (Elixir of Immortality stays a card, same ruling Thor's Deathless Rage and Anubis's Osiris's Rebirth got, confirmed once for all three at full-depth review per [GDD §3.4](GameDesignDocument.md)'s closing note).

**Real judgment call, confirmed via direct question to the user (recommended option taken):**

**Form-changing resolves as a deterministic cycle** (Mortal → Beast → Immortal → Mortal), not a player-facing choice. Several cards say "change to a different Form" or (the starter, Ruyi Strike) "change to a Form of your choice" — genuine player choice among 2-3 options, the same class of gap as Anubis's deferred "choose one" cards. Unlike Anubis's case, cycling resolves nearly the *entire* affected set at once rather than needing a per-card reword-or-drop call: Ruyi Strike's "of your choice" becomes "Change Form" (near-lossless — still the signature Form-teaching starter, just deterministic instead of picker-driven), and only one card in the full 75 (Wandering Form, "any Form including current," which breaks the cycle rule since staying in place isn't a cycle step) doesn't fit and gets swapped for a near-identical alternative already in the same pool (Ever-Changing Form). Full reasoning and the exact card-level resolution live in [SunWukong-CardDraft.md](Characters/SunWukong-CardDraft.md).

**Content curated/drafted this session:**
1. **[SunWukong-CardDraft.md](Characters/SunWukong-CardDraft.md)** — a 25-card curated slice (10 starter + 5 per archetype path), mirroring the other three Blessings' shape exactly. Havoc in Heaven / Ascension of the Sage / 72 Changes serve as each path's finisher. Deliberately Power-card-free: almost every Power in the full 75 needs a `TriggerEvent` the current system doesn't have yet ("whenever you change Form," "whenever you deal damage" — same class of gap that held back Syncretism's Aegis of the Hunt in M5), so none were forced in.
2. **[MinimalSampleSet-Chinese.md](Enemies/MinimalSampleSet-Chinese.md)** — 3 new enemies (Heavenly Soldier, Nine-Tailed Fox Spirit, Yaksha Swarm) at the same power tiers as the other three sets' counterparts. **Debuff variety is exhausted** (Greek/Norse/Egyptian already cover all 3 universal debuffs from M1 — Drained/Sundered/Exposed — and inventing a 4th is out of scope until real enemy design calls for it), so this set's second enemy pivots to `IntentType.Buff` instead (gains Strength on itself) rather than repeating a debuff. Own judgment call, not put to the user: `IntentType.Buff` already exists in `CombatResolver` but has zero live coverage across the other three sets, so this is a clean way to keep the "no repeat" pattern going by exercising an untested intent branch instead of reusing a tested one.

**New naming decisions:** none. First milestone where no new `CardTag` was needed (Thor added `StormGenerating`, Anubis added `Order`/`Chaos`) — nothing in Wukong's curated slice or full 75 filters on "is this a Form-changing card" the way Pathfinder filters on `Shot`; Form-reading cards check current Form *state*, not a card tag.

**Deliberately out of scope for M7** (per the roadmap table — Form/cards/enemies/passive-tree only): Chinese relics including Wukong's signature/starter relic, his Player/Minor/Major Enhancements, and a Chinese Shop instance name. Same one-milestone-later pattern as Norse and Egyptian relics after M5/M6's combat slices. Also out of scope: any Syncretism pairing involving Wukong — a fourth Blessing doesn't automatically need new fusion content, same reasoning M6 already established.

**Sequencing** — 4 branches (no Syncretism branch, same reasoning as M6):
1. `feature/form-resource` — `Player.Form` (enum `Mortal`/`Beast`/`Immortal`, resets on `PrepareForCombat`) + a deterministic `ChangeFormEffect` (cycles Mortal→Beast→Immortal→Mortal) + "changed Form this turn" tracking (same shape as `ShotsPlayedThisTurn`). Structurally the biggest resource branch yet: unlike Volley/Storm/Scale, which only affect cards that explicitly compose a resource-reading `CardEffect`, Form's Beast/Immortal modifiers apply globally to *all* Attacks/Skills/Strength-gains/Block-gains while active — this branch has to reach into `DealDamageEffect`, `GainBlockEffect`, `ApplyStatusEffect`, and `CombatResolver.PlayCard`'s Skill-cost calculation, not just add new standalone effect classes. Exact implementation shape (e.g., reusing `ConditionalDoubleHitDamageEffect`'s existing "hits twice" precedent from Artemis's kit vs. a new mechanism) is implementation-level, worked out via TDD in the slice itself, not pre-designed here.
