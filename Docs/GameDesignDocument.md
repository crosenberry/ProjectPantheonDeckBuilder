...
- Map & Run Structure (Section 12): exact node count per stage, exact stage count per run, and Rest Site's heal percentage — rule shapes are settled, numbers aren't, same standing deferral as everywhere else.
- Whether an Elite combat tier (harder optional fight, better rewards) gets added, and if so when — deliberately deferred alongside the rest of "full enemy/boss design for all 4 mythologies" above, not part of M4.

Resolved this pass: card scale (75/character + ~70 colorless), no-rarity relics confirmed as current design (not permanently ruled out), Player Enhancement trigger type (combat-count), all 4 keyword mechanics formalized into rule shapes, Minor Enhancement risk mechanic (3-tier Cautious/Bold/Defiant Offering system), trial mythology randomized independent of stage, next-stage mythology selection (default random + extensible "Prophecy" family of Major Enhancements, 2 variants per mythology so far, each with a unique minor bonus on top of a shared reveal-2-pick-1 core), merchant/shop node confirmed to exist and designed (Section 7), **all 4 Blessings taken to full 75-card depth** (Artemis, Thor, Anubis, Sun Wukong), safety-net Rare pattern ruling (stays a card, not a relic; not forced onto Artemis), Sun Wukong's Beast Form × multi-hit ruling (double-hit applies to single-hit Attacks only), **Core Combat Vocabulary formalized as its own section** (Section 10), **passive tree rule shape** (Mythos currency, one tree per Blessing built on existing archetype paths, depth-over-access node philosophy, Hades-style respec-able choice pairs, Mythic card tier), **all 4 Blessings' passive trees built** (17 nodes each), confirming "Mythic capstones can deliberately bend a Blessing's core rule" as a recurring, intentional cross-tree design principle, **enemies confirmed tied to stage mythology** (not decoupled like trials), and a **minimal Greek enemy sample set built** to unblock a first playable slice. Also resolved: **Map & Run Structure formalized as its own section** (Section 12) — the in-stage node graph's node types (Combat, Chest, Shop, Trial, Rest, Boss), a new **Rest Site** node type confirmed to exist (heal-only, kept distinct from the Godly Trial's upgrade purpose), a run confirmed to be a **fixed sequence of multiple stages** chained via Rifts rather than a single stage, and Elite combat tier deliberately deferred (not part of M4).

## 12. Map & Run Structure

Covers the layer that connects everything Sections 4-7 already built (relics, enhancements, shop) into an actual playable sequence, per the M4 design session.

**Map**: the branching node graph within a single Stage, same standard genre shape as every other game in the space — Hook 3 (a spatial/structural twist on this) was explicitly parked, not pursued (see [DifferentiationHooks.md](DifferentiationHooks.md)), so no twist here. "Map" stays the generic engineering term, same reasoning as Block/Shop/Curse in Section 10 — a structural/UI-level word every genre entry already uses, not something that needs mythology-flavored branding. Exact node count and path-branching shape per stage is deferred to tuning, same as everywhere else.

**Node types** (all generic/universal, not mythology-flavored — flavor lives in what's inside a node, e.g. Hermes' Exchange as the Greek Shop instance, not in the node label itself):
- **Combat**: a regular fight against the stage mythology's enemy roster (Section 3 sample sets).
- **Chest**: opens a mythology-flavored relic (Section 4).
- **Shop**: the Merchant node (Section 7) — Hermes' Exchange for Greek.
- **Trial**: the Godly Trial (Section 5.2) — Minor Enhancement upgrades via the Cautious/Bold/Defiant Offering system.
- **Rest Site** *(new this section)*: heal-only node, kept deliberately separate from Trial rather than folded into it — Trial already owns the upgrade purpose (StS's campfire dual-purpose heal-or-upgrade choice is deliberately not the shape used here, to keep the two node types' jobs distinct). Exact heal amount is deferred to tuning, same as every other number in this document. Doesn't resolve the still-open potion-equivalent question (Section 2) — a Rest Site is a map node you walk to, a potion (if it ends up existing) would be a portable consumable; they may end up serving overlapping purposes once that question is revisited, but aren't the same thing.
- **Boss**: a stage's climactic fight (2 randomized boss options per stage mythology, Section 4). Defeating one drops a **Soul**, opening the relic reward screen already described in Section 4 — not a separate map node the player walks to, it's what winning the Boss node triggers.

**Rift**: unchanged from Section 5.3 — the portal/gateway interaction after a Boss is defeated, not a node on the branching path itself. Delivers the stage's Major Enhancement and the next-stage mythology choice (default random reveal-2-pick-1, or an owned Prophecy-family variant's upgraded version).

**Run**: a full run is a **fixed sequence of multiple Stages**, chained one after another via each Stage's ending Rift — not a single Stage. This is what actually earns M4's "start-to-finish run, win or lose" bar and the `v0.2` milestone tag; a single-Stage loop was considered and rejected as too light a bar for that tag. Exact stage count is deferred to tuning like every other number here — the shape (multiple stages, chained via Rifts, ending in the final stage's Boss) is what's confirmed now. A run ends in victory when the final stage's Boss is defeated, or in defeat whenever the player's HP reaches 0 at any point — no new terminology needed for either outcome beyond what Section 8 already established (Mythos awards partial credit on a run that ends in death, Hades'-Darkness-style).

**Elite combat tier**: deliberately **not** part of this milestone. Would need at least one new, stronger enemy authored per mythology — better scoped alongside the already-deferred "full enemy/boss design for all 4 mythologies" item (Section 11) than bundled into run-structure work. Regular Combat nodes use the existing minimal sample sets (Section 3) for now.

- **[OPEN]**: exact node count per stage, exact stage count per run, path-branching shape (how many parallel paths, how they converge), and Rest Site's heal percentage — rule shapes settled, numbers/layout details aren't.
